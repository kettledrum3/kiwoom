...
def generate_daily_report_html(market):
    ...
    try:
        ...
        deposit_html = f"<div style='background-color: #f8f9fa; padding: 15px; border-radius: 10px; border-left: 5px solid #2ecc71; margin: 15px 0;'><span style='color: #2c3e50; font-weight: bold;'>💵 계좌 예수금:</span> <b>{currency}{cash_pool_fmt}</b></div>"
    except Exception as e:
        logger.error(f"[Email] 일일보고 예수금 조회 실패: {e}")
        deposit_html = "<div style='background-color: #f8f9fa; padding: 15px; border-radius: 10px; border-left: 5px solid #e74c3c; margin: 15px 0;'><span style='color: #2c3e50; font-weight: bold;'>💵 계좌 예수금:</span> 조회 실패</div>"
    ...
    html += """
        <br><br>
        <p style="font-size: 12px; color: #888;">본 메일은 키움 시스템에서 자동 발송되었습니다.</p>
    </body>
    </html>
    """
    return html
...
def job_send_daily_report(market):
    """스케줄러에서 호출할 작업"""
    subject = f"🔔 [키움CAVR] {market} 시장 장 종료 보고서"
...
def generate_monthly_report_html() -> MIMEMultipart:
    ...
    <body style="font-family: Arial, sans-serif;">
        <h2 style="color: #2e6c80;">📅 [키움CAVR] 월간 투자 결산 리포트 ({month_str})</h2>
...
def job_send_monthly_report():
    ...
    subject = f"📊 [키움CAVR] {month_str} 월간 투자 결산 리포트" # Subject should be a string
