...
def get_trade_mode() -> str:
    """
    현재 설정된 거래 모드 반환 ('REAL' or 'MOCK')
    기본값은 'REAL'이며, DB system_config에 TRADE_MODE 설정이 있는 경우 이를 따름.
    설정이 없는 경우 .env 또는 os.getenv의 값을 조회.
    """
    try:
        mode = get_config("TRADE_MODE")
    ...
    try:
        ...
        common_env_path = os.path.join(BASE_DIR, "env", ".env")
        if os.path.exists(common_env_path):
            env_config = dotenv_values(common_env_path)
            mode = env_config.get("TRADE_MODE")
            if mode in ["REAL", "MOCK"]:
                return mode
    except Exception:
        pass
