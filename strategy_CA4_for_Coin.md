...
### 3.4. 암호화폐용 리버스 모드 (Reverse Mode) 명세
* **진입 조건:** $T \ge a - 1$ 시점 자동 진입 (`reverse_cycle_count = 0`).
* **지속성 보장:** 리버스 매도로 인해 $T$값이 감소하더라도 종료(회귀) 조건을 충족하기 전까지는 **리버스 모드를 100% 유지**.
* **리버스 별지점 기준가 ($MA5_{8H}$):** 주식의 '5거래일 종가 평균' 대신, **'직전 5개의 8시간 봉 종가 평균(Average of last 5 candles of 8H)'**을 사용합니다.
  * 업비트 (`/v1/candles/minutes/240` 또는 `/v1/candles/days` 활용하여 8시간 단위 종가 파싱)
  * 빗썸 (`/public/candlestick` 활용하여 8시간 캔들 데이터 추출)
* **매도 분할 비율:** $1 / (a/2)$ 적용 (40분할 시 5%, 20분할 시 10%, `math.floor` 처리). 소수점 코인 수량은 소수점 4자리까지 계산.
* **주기별 분기 실행 규칙:**
  * **첫날 (1회차, `reverse_cycle_count == 0`):**
    * **매도:** 전체 보유 수량의 $1 / (a/2)$ 분량을 **시장가(Market)**로 강제 매도.
    * **매수:** **매수 주문은 일체 없음**.
  * **2회차 이후 (`reverse_cycle_count >= 1`):**
    * **무한 매도:** 직전 5개 8시간 봉 종가평균($MA5_{8H}$) 가격으로 직전 보유 수량의 $1 / (a/2)$ 분량 **지정가(LOC 모사) 매도**.
    * **쿼터 매수:** $MA5_{8H}$ 아래(예: $MA5_{8H} \times 0.9999$)에서 가용 Pool의 25%($\text{Pool} \times 0.25$)를 **지정가(LOC 모사) 매수**.
* **체결 시 T값 갱신 수식:**
...
* **종료(회귀) 조건:**
  * 8시간 봉 마감 시점의 현재가(종가)가 평단가 대비 **(-)목표수익률** 이상으로 회복 시 일반 모드로 복귀.
  * **탈출 기준가** = $\text{평단가} \times (1.0 - \text{목표수익률}(\%))$
  * 복귀 시 리버스 모드의 $T$값과 V4.0 유동적 매수금 공식($\text{1회 매수금} = \frac{\text{가용 Pool}}{a - T}$)을 그대로 계승하여 사이클 연속성 유지.
* **가드 로직:**
  * 리버스 모드 가동 중에는 일반 모드의 75% 지정가 매도 모니터링을 건너뜀.
